...
module.exports = ({
  ...
  //   the type of the argument given, and on whether the Swarm address has a
  //   directory or a file.
    ...
    isAvailable("http://localhost:8500").then(isAvailable =>
      isAvailable
        ...
        : downloadBinary(swarmSetup.binPath, swarmSetup.archives)
          .onData(data => (swarmSetup.onProgress || (() => {
          }))(data.length))
  ...
  const fromString = string =>
    bytes.toUint8Array(String.fromString(string));
  ...
  const createMutableResource = swarmUrl => payLoad => {
    const response = request(`${swarmUrl}/bzz-resource:/`, {
      body: JSON.stringify(payLoad),
      method: "POST"
    });
    return response.then(MRU_MANIFEST_KEY => MRU_MANIFEST_KEY.slice(1, MRU_MANIFEST_KEY.length - 1));
  };
  ...
  const uint8ToHex = num => {
    const byte = web3_utils.toHex(num % 256).slice(2);
    if (byte.length === 0) {
      return "00";
  ...
  const uint16ToLittleEndianHex = num => (
    "0x" + uint8ToHex(num % 256) + uint8ToHex(0xff & (num >> 8))
  );

  const uint32ToLittleEndianHex = num => (
    "0x" + uint8ToHex(num % 256) + uint8ToHex(0xff & (num >> 8)) + uint8ToHex(0xff & (num >> 16)) + uint8ToHex(0xff & (num >> 24))
  );

  const getSha3 = function getSha3(period, version, rootAddr, metaHash, multihash, data) {
    return web3_utils.soliditySha3(
      {t: 'bytes', v: '0x4900'},
      {t: 'bytes', v: uint16ToLittleEndianHex((data.length - 2) / 2)},
      {t: 'bytes', v: uint32ToLittleEndianHex(period)},
      {t: 'bytes', v: uint32ToLittleEndianHex(version)},
      {t: 'bytes32', v: rootAddr},
      {t: 'bytes32', v: metaHash},
      {t: 'uint8', v: multihash ? 1 : 0},
      {t: 'bytes', v: data});
  };
