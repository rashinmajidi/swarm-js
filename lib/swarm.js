...
module.exports = function (_ref) {
    ...
    var fromString = function fromString(string) {
        return bytes.toUint8Array(String.fromString(string));
    };
    ...
    var createMutableResource = function createMutableResource(swarmUrl) {
        return function (payLoad) {
            var response = request(swarmUrl + "/bzz-resource:/", {
                body: JSON.stringify(payLoad),
                method: "POST"
            });
            return response.then(function (MRU_MANIFEST_KEY) {
                return MRU_MANIFEST_KEY.slice(1, MRU_MANIFEST_KEY.length - 1);
            });
        };
    };

    var retrieveMutableResource = function retrieveMutableResource(swarmUrl) {
        return function (MRU_MANIFEST_KEY, n, m) {
            return request(swarmUrl + "/bzz-resource:/" + MRU_MANIFEST_KEY, {
                method: "GET"
            });
        };
    };

    var updateMutableResource = function updateMutableResource(swarmUrl) {
        return function (payLoad, MRU_MANIFEST_KEY) {
            var response = request(swarmUrl + "/bzz-resource:/" + MRU_MANIFEST_KEY + "/meta", {
                method: "GET",
                headers: { "Cache-Control": "no-cache" }
            });
            return response.then(function (metaStr) {
                var meta = JSON.parse(metaStr);
                if (typeof payLoad.period === 'undefined') payLoad.period = meta.period;
                if (typeof payLoad.version === 'undefined') payLoad.version = meta.version;
                if (typeof payLoad.metaHash === 'undefined') payLoad.metaHash = meta.metaHash;
                if (typeof payLoad.rootAddr === 'undefined') payLoad.rootAddr = meta.rootAddr;
                if (typeof payLoad.multiHash === 'undefined') payLoad.multiHash = meta.multiHash;
                var dataToSign = getSha3(payLoad.period, payLoad.version, payLoad.rootAddr, payLoad.metaHash, payLoad.multiHash, payLoad.data);
    ...
    var uint8ToHex = function uint8ToHex(num) {
        var byte = web3_utils.toHex(num % 256).slice(2);
        if (byte.length === 0) {
            return "00";
    ...
    var uint16ToLittleEndianHex = function uint16ToLittleEndianHex(num) {
        return "0x" + uint8ToHex(num % 256) + uint8ToHex(0xff & num >> 8);
    };

    var uint32ToLittleEndianHex = function uint32ToLittleEndianHex(num) {
        return "0x" + uint8ToHex(num % 256) + uint8ToHex(0xff & num >> 8) + uint8ToHex(0xff & num >> 16) + uint8ToHex(0xff & num >> 24);
    };

    var getSha3 = function getSha3(period, version, rootAddr, metaHash, multihash, data) {
    ...
    return {
        ...
        createMutableResource: createMutableResource,
        retrieveMutableResource: retrieveMutableResource,
        updateMutableResource: updateMutableResource,
        pick: pick,
        hash: hash,
        fromString: fromString,
        toString: toString
